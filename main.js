...
function applyI18nTexts() {
    ...
    byId('eventSoldOut').options[0].text = t('soldOutNo');
    byId('eventSoldOut').options[1].text = t('soldOutYes');
...
function openEventView(eventItem) {
    ...
    if ((eventItem.event_mode || 'online') === 'offline') {
        ...
        if (eventItem.ticket_url) {
            byId('eventViewTicketWrap').innerHTML = `<a href="${eventItem.ticket_url}" target="_blank" rel="noopener" class="ticket-cta"><span class="ticket-icon" aria-hidden="true">&#127915;</span>${t('getTicketNow')}</a>`;
        }
...
function openEventDialog(eventItem = null, prefillDate = null) {
    ...
    byId('eventSoldOut').value = isSoldOut(eventItem) ? '1' : '0';
...
byId('eventForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        form.append('sold_out', byId('eventSoldOut').value === '1' ? '1' : '0');
