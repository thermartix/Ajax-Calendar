...
async function api(path,options={}){const r=await fetch(path,{headers:{'Content-Type':'application/json'},...options});const d=await r.json();if(!r.ok||d.success===false)throw new Error(d.message||'Request failed');return d;}
let countries=[];
...
async function loadCountries(){const d=await api('includes/api/countries.php');countries=d.countries;byId('countriesText').value=countries.map(c=>`${c.code}|${c.name}`).join('\n');}
function countryOptions(selected){return countries.map(c=>`<option value="${c.id}" ${Number(selected)===Number(c.id)?'selected':''}>${c.name}</option>`).join('');}

async function loadUsers(){const d=await api('includes/api/admin_users.php');const root=byId('usersRoot');root.innerHTML='';d.users.forEach(u=>{if(u.role==='admin') return; const wrap=document.createElement('div');wrap.className='event-card';wrap.style.marginBottom='8px';wrap.innerHTML=`<strong>${u.username}</strong> (${(u.first_name||'')+' '+(u.last_name||'')})<br><label>Approved <input type="checkbox" id="ap_${u.user_id}" ${u.is_approved?'checked':''}></label><br><label>Primary country</label><select id="pc_${u.user_id}">${countryOptions(u.country_id)}</select><br><label>Allowed countries (comma IDs)</label><input id="ac_${u.user_id}" value="${(u.allowed_country_ids||[]).join(',')}"><br><button id="su_${u.user_id}" class="accent">Save user</button>`;root.appendChild(wrap); document.getElementById(`su_${u.user_id}`).onclick=async()=>{const allowed=(document.getElementById(`ac_${u.user_id}`).value||'').split(',').map(s=>Number(s.trim())).filter(n=>n>0);await api('includes/api/admin_user_update.php',{method:'POST',body:JSON.stringify({user_id:u.user_id,is_approved:document.getElementById(`ap_${u.user_id}`).checked?1:0,country_id:Number(document.getElementById(`pc_${u.user_id}`).value),allowed_country_ids:allowed})});byId('adminMsg').textContent='User updated';};});}

async function init(){
  const session=await api('includes/api/auth_session.php');
  if(!session.loggedIn){location.href='login.php';return;}
